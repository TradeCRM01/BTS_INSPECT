...
import {
  CONTRACT_VISIT_REMINDER_CLIENT_COLUMNS,
  ...
  type ContractVisitReminderDecision,
} from '../../lib/contractVisitReminder';
import { deliverContractVisitReminder, loadContractVisitReminderBundle } from '../../lib/contractVisitReminderDeliver';
...
import {
  CONTRACT_CLIENT_ATTACH_NO_CLIENTS,
  attachContractClient,
  contractClientAttachRow,
} from '../../lib/attachContractClient';

/** Honest no_email miss — write the address on this dialog. */
export const CONTRACT_VISIT_REMINDER_NO_EMAIL_FIELD =
  'This client has no email. Add one below before you send.';

export function ContractVisitReminderDialog({
  contractId,
  company,
  onClose,
  onSent,
  ...
  const [bundle, setBundle] = useState<ContractVisitReminderBundle | null>(null);
  const [decision, setDecision] = useState<ContractVisitReminderDecision | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setErr('');
      try {
        const loaded = await loadContractVisitReminderBundle(contractId, company);
        if (cancelled) return;
        setBundle(loaded);
        setDecision(decideContractVisitReminder({ bundle: loaded, companyId: company.id, mode: 'manual' }));
        setClientEmailDraft(loaded.client?.email ?? '');
        setClientPhoneDraft(loaded.client?.phone ?? '');
      } catch (e) {
        if (!cancelled) setErr(e instanceof Error ? e.message : 'Could not load this contract.');
      } finally {
  ...
  const contractClientId = bundle?.contract?.client_id ?? null;
  ...
  const attachClientsQuery = useQuery<{ id: string; name: string }[]>({
    queryKey: ['contract-attach-clients', company.id],
    queryFn: async () => {
  ...
  const attachRow = contractClientAttachRow({
    contractClientId,
    companyClients: contractClientId
      ? []
  ...
  const handleAttach = async () => {
    if (!bundle?.contract || attachRow.kind !== 'pick') return;
    setSavingAttach(true);
    setErr('');
    try {
      const result = await attachContractClient({
      ...
      const clientRes = await supabase
        .from('clients')
        .select(CONTRACT_VISIT_REMINDER_CLIENT_COLUMNS)
        .eq('id', result.clientId)
        .maybeSingle();
      if (clientRes.error) throw clientRes.error;
      const attached = (clientRes.data ?? null) as ContractVisitReminderClient | null;
      const picked = attachRow.clients.find(c => c.id === result.clientId);
      const next: ContractVisitReminderBundle = {
        ...bundle,
        contract: { ...bundle.contract, client_id: result.clientId },
        client: attached ?? (picked
          ? { id: picked.id, name: picked.name, email: null, phone: null, address: null }
          : null),
      };
      setBundle(next);
      setDecision(decideContractVisitReminder({ bundle: next, companyId: company.id, mode: 'manual' }));
      setClientEmailDraft(next.client?.email ?? '');
      setClientPhoneDraft(next.client?.phone ?? '');
      setClientAttachDraft('');
      void queryClient.invalidateQueries({ queryKey: ['service-contracts'] });
  ...
  const handleSend = async () => {
    ...
    setErr('');
    try {
      const result = await deliverContractVisitReminder({
        contractId,
        company,
      });
      if (!result.ok) {
      ...
      setErr(e instanceof Error ? e.message : 'Could not send the reminder.');
    } finally {
      setSending(false);
    }
  };
  ...
  const blockerMessage = noEmailMiss
    ? CONTRACT_VISIT_REMINDER_NO_EMAIL_FIELD
    : noClientsNamedMiss
      ? CONTRACT_CLIENT_ATTACH_NO_CLIENTS
      : (decision && !decision.ok ? decision.message : '');
  const visitLabel = bundle?.contract
    ? [bundle.contract.title, bundle.contract.contract_number].filter(Boolean).join(' · ')
    : '';
      ...
      <div className="hub-invoice-send">
        <div className="hub-invoice-send-head">
          <div className="min-w-0">
            <h2 className="hub-invoice-send-title">Remind client</h2>
            {visitLabel ? <p className="hub-invoice-muted mt-1">{visitLabel}</p> : null}
          </div>
        </div>
        ...
        <div className="hub-invoice-send-body">
            ...
            <>
              <div className="hub-invoice-send-tos">
                <div className="hub-invoice-send-field">
                  ...
                  <p className="hub-invoice-muted">{decision.toName} — already on the contract.</p>
                </div>
                <div className="hub-invoice-send-field">
          ...
          {!loading && !ready && (
            <>
              <p className="hub-invoice-err">{blockerMessage || err || 'This reminder cannot be sent yet.'}</p>
