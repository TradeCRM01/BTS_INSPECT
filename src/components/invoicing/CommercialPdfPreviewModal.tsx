...
export function CommercialPdfPreviewModal({ data, onClose }: CommercialPdfPreviewModalProps) {
  ...
  return (
    <OverlayPortal>
      <div className={`overlay-backdrop${quoteLook ? ' hub-quote-pdf-preview' : ''}`}>
        <div
          className={`overlay-panel-xl flex flex-col max-h-[92vh] ${quoteLook ? 'hub-quote-pdf-sheet' : 'border border-[#E5E7EB]'}`}
          onClick={e => e.stopPropagation()}
        >
          <div className={`flex items-center justify-between px-5 py-3 shrink-0 ${quoteLook ? 'hub-quote-pdf-head' : 'border-b border-[#E5E7EB]'}`}>
            <div className="flex items-center gap-2">
              <FileText size={16} className="text-[#2E75B6]" />
              <h2 className={`text-base font-semibold ${quoteLook ? 'text-[#0A2540]' : 'text-[#1A1A1A]'}`}>
                {quoteLook ? 'Quote preview' : 'Document preview'}
              </h2>
              <span className={`text-xs ${quoteLook ? 'text-[#5B6B7C]' : 'text-[#9CA3AF]'}`}>{data.docNumber}</span>
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
          ...
          <div className={`flex-1 min-h-0 p-3 ${quoteLook ? 'bg-[#F5F0E6]' : 'bg-[#E5E7EB]'}`}>
            {loading && (
