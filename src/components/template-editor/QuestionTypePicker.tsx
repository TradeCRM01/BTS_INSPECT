...
export function QuestionTypePicker({ onSelect, onClose, title = 'Add Question' }: {
  ...
  return (
    <div className="overlay-backdrop" onClick={onClose}>
      <div
        className="overlay-panel-md w-full sm:max-w-lg flex flex-col"
        style={{ maxHeight: 'min(90dvh, 100%)' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#E5E7EB] shrink-0 bg-white">
          <h3 className="font-semibold text-[#1A1A1A]">{title}</h3>
          <button
            type="button"
            onClick={onClose}
            className="text-[#4A5568] hover:text-[#1A1A1A] p-1 rounded min-w-10 min-h-10 flex items-center justify-center"
          >
            <X size={18} />
          </button>
        </div>
