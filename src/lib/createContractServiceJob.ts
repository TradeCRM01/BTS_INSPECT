...
export function missContractJobMessage(reason: ContractJobBlocker): string {
  switch (reason) {
    ...
    case 'no_creator':
      return 'This company has no team member to own the job — job was not created.';
  }
}
...
/** Oldest admin, else the first member. Never invents a profile id. */
export function pickContractJobCreator(
  members: Array<{ id?: string | null; role?: string | null }> | null | undefined,
): string | null {
  const rows = (members ?? [])
    .map(m => ({ id: (m.id ?? '').trim(), role: (m.role ?? '').trim() }))
    .filter(m => m.id);
  if (rows.length === 0) return null;
  return (rows.find(m => m.role === 'admin') ?? rows[0]).id;
}

export function selectAutoGenerateDueContracts(
  contracts: ContractVisit[],
  companyId: string,
  now = new Date(),
): ContractVisit[] {
