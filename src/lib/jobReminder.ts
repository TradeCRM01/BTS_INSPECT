...
/**
 * SMS rides the same job-reminder invoke as email (tray, invoice Send, due=tomorrow, due=today, due=overdue, due=tickets).
 * Twilio secrets stay on the edge. Email sent-at / chased_at is unchanged if SMS misses.
 */
export const JOB_REMINDER_SMS_PIPE = [
  'supabase.functions.invoke job-reminder (same body as email: jobId / inspectionId / invoiceId / reportId / quoteId / purchaseOrderId / contractId / due=tomorrow / due=today / due=contract / due=overdue / due=tickets)',
  ...
  'client_reminder_sent_at / invoice status / invoices.chased_at / reports.sent_at / due_reminder_sent_at / service_reminder_sent_at / member_tickets.reminder_sent_at follow email 2xx only',
] as const;
