...
function lookCss(): string {
  ...
  const lookStart = css.indexOf('/* Reports list document only.');
  const lookEnd = css.indexOf('/* End reports list document', lookStart);
  return css.slice(lookStart, lookEnd);
}

describe('reports list laptop LOOK — quote paper, overflow on the sheet', () => {
  it('paints the reports list as one Looplet paper, not a stacked admin list', () => {
    const page = src('src/pages/ReportsListPage.tsx');
    const css = lookCss();

    expect(page).toContain('hub-reports-list-doc');
    expect(page).toContain('hub-reports-list-bar');
    expect(page).toContain('hub-reports-list-mark');
    expect(page).toContain('hub-reports-list-body');
    expect(page).toContain('hub-reports-list-whisper');
    expect(page).toContain('hub-reports-list-tools');
    expect(page).toContain('hub-reports-sheet');
    expect(page).toContain('hub-reports-row');
    ...
    expect(page).toContain('placeReportsListMore');
    expect(page).toContain('inkFloor');
    expect(page).not.toContain('hub-reports-lede');
    expect(page).not.toContain('>Open<');
    expect(page).not.toContain('hub-reports-list-mark">Reports');
    ...
    expect(page).toContain('--hub-reports-list-more-shift');
    expect(page).toContain("look') === REPORTS_LIST_LOOK");
    ...
    expect((page.match(/hub-reports-list-more/g) ?? []).length).toBeGreaterThanOrEqual(1);
    expect((page.match(/MoreHorizontal/g) ?? []).length).toBeGreaterThanOrEqual(1);

    expect(css).toContain('.hub-reports-list-doc');
    expect(css).toContain('.hub-reports-list-bar');
    expect(css).toContain('.hub-reports-list-whisper');
    expect(css).toContain('.hub-reports-list-tools');
    ...
    expect(css).toContain('.hub-reports-list-more.is-flip');
    expect(css).toContain('.hub-reports-list-more.is-shift');
  ...
  it('does not rewrite persist, search, or convert writes', () => {
    ...
    expect(page).toContain('filterReportsForSearch');
    expect(page).toContain('filterReportsByStatus');
    expect(page).toContain('Search site, report, #0042');
  ...
  it('stays off landing, quotes, week-board, job sheet, jobs list, Relovi, and AppShell', () => {
    ...
    expect(page).not.toContain('hub-jobs-list-doc');
    expect(page).not.toContain('hub-week-document');
    expect(src('src/pages/MarketingPage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/pages/QuotesPage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/pages/JobDetailPage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/pages/JobsPage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/pages/ClientsPage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/pages/SchedulePage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/pages/InvoicesPage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/pages/DashboardPage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/pages/LoginPage.tsx')).not.toContain('hub-reports-list-doc');
    expect(src('src/components/layout/AppShell.tsx')).not.toContain('hub-reports-list-doc');
  });
});

describe('reports list laptop LOOK frames', () => {
  it('covers 1280 document, overflow, and quote-paper reference', () => {
    for (const rel of [
      'docs/look/reports-list-laptop-1280-document.png',
      'docs/look/reports-list-laptop-1280-overflow.png',
