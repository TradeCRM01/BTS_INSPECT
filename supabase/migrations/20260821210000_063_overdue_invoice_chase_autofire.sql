-- Thin due=overdue hop on the existing Perth invoke.
-- Same cron names and times. No new table. No new cron stack.
-- Tomorrow jobs and due inspections stay as signed.
-- Overdue unchased invoices ride job-reminder (email + SMS) beside them.
...
COMMENT ON FUNCTION public.invoke_job_client_reminders() IS
  'pg_cron Perth auto-fire: pg_net POST job-reminder due=tomorrow (jobs), due=today (inspections), and due=overdue (unchased invoices). SMS rides the edge beside email.';
