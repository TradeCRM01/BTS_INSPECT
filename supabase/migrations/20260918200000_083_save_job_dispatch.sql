...
COMMENT ON COLUMN jobs.dispatch_ready IS 'planning vs ready-to-dispatch. Soft vs hard requirement gaps.';
COMMENT ON COLUMN jobs.dispatch_version IS 'incremented by save_job_dispatch. Not last-write-wins.';
...
CREATE OR REPLACE FUNCTION public.dispatch_time_to_minutes(t text)
RETURNS integer
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    ...
    ELSE (split_part(t, ':', 1)::integer * 60)
      + coalesce(nullif(split_part(t, ':', 2), '')::integer, 0)
  END;
$$;

REVOKE ALL ON FUNCTION public.dispatch_time_to_minutes(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.dispatch_time_to_minutes(text) TO authenticated, service_role;
...
BEGIN
  ...
  v_start := coalesce(nullif(p->>'start_time', ''), v_job_start);
  v_end := coalesce(nullif(p->>'end_time', ''), v_job_end);
